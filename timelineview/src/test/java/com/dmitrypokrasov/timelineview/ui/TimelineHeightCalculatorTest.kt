...
class TimelineHeightCalculatorTest {
    ...
    fun `height grows with measured multiline text`() {
        ...
        val layout = TimelineLayout(
            steps = listOf(
                TimelineLayoutStep(
                    step = TimelineStepData(title = "Title", description = "Description", progress = 10),
                    titleX = 0f,
                    titleY = 20f,
                    titleWidth = 100,
                    descriptionX = 0f,
...
private class FakeMeasuringRenderer(
    ...
    override fun measureTitleHeight(title: CharSequence, maxWidth: Int, align: Paint.Align): Int =
        if (title.isBlank()) 0 else titleHeight

    override fun measureDescriptionHeight(description: CharSequence, maxWidth: Int, align: Paint.Align): Int =
        if (description.isBlank()) 0 else descriptionHeight
